...
class ListView extends BaseView
{
    ...
    public function btnNewUrl(): string
    {
        ...
        $params = [];
        foreach (
            DataBaseWhere::getFieldsFilter($this->where)
            as $key => $value
        ) {
            if ($value !== false) {
                $params[] = $key . '=' . $value;
    ...
    public function export(&$exportManager, $codes): bool
    {
        ...
        if (
            is_array($codes) &&
            count($codes) > 0 &&
            $this->model instanceof ModelClass
        ) {
    ...
    public function loadData(
        ...
        $this->count = is_null($this->model)
            ? 0
            : $this->model->count($this->where);
    ...
    protected function assets()
    {
        AssetManager::add(
            'js',
            FS_ROUTE . '/Dinamic/Assets/JS/ListView.js?v=2'
        );
    }
    ...
    private function loadTotalAmounts()
    {
        ...
        if ($isAdmin) {
            $tableName =
                count($this->cursor) > 1 &&
                method_exists($this->model, 'tableName')
            ...
            foreach ($this->getColumns() as $col) {
                ...
                if (
                    $col->hidden() ||
                    false === $col->widget->showTableTotals()
                ) {
                    continue;
                }
                ...
                if (
                    false ===
                    array_key_exists($col->widget->fieldname, $modelFields)
                ) {
                    continue;
                }
    ...
    private function processFormDataLoad(Request $request)
    {
        ...
        if ('' !== $this->query) {
            ...
            $this->where[] = new DataBaseWhere(
                $fields,
                ToolBox::utils()::noHtml($this->query),
                'XLIKE'
            );
        }
